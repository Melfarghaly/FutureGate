...
if(!function_exists("get_upload_folder")){
	function get_upload_folder($folder = ""){
		$path = APPPATH."../assets/uploads/user" . sha1(session("uid"))."/";
		if ($folder != "") {
			$path = APPPATH."../assets/uploads/". $folder."/";
		}
		if (!file_exists($path)) {
			$uold     = umask(0);
	    	mkdir($path, 0777);
...
if (!function_exists('get_link_file')) {
	function get_link_file($file_name, $folder = ""){
		if ($folder != "") {
			$result = BASE."assets/uploads/".$folder."/".$file_name;
		}else{
			$result = BASE."assets/uploads/user".sha1(session("uid"))."/".$file_name;
		}
		return $result;
	};
}
