    ...
    <?php
        if($lang=='en') 
            $cat_name = strip_tags($category->name_en);
        else
            $cat_name =strip_tags($category->name);

             
      $data_link = (object)array(
        'link'  => cn(strip_tags($category->url_slug)),
        'name'  => strip_tags($cat_name)
      );
        ...
        <div class="row">
          <div class="col-md-12 text-center">
            <div class="pk-header">
              <?php
                if($lang=='en') 
                    $replace_text = '<strong>'.strip_tags($category->name_en).'</strong>';
                else
                ...
                <div class="card">
                  <div class="text-center">
                    ...
                    <div class="text-center order_button">
                      ...
                      <!--  <button class="btn btn-success shoppingCart" onclick="addToCart(<?php echo strip_tags($row->id); ?>)"><i class="fe fe-shopping-cart"></i></button>-->
                        <?php 
                          if ($row->status) {
                        ?> 
          ...
          if($lang=='ar'){
            if (isset($category->content) && $category->content) {
             ...
             if (isset($category->content_en) && $category->content_en) {
              echo '<div class="col-md-12 package-details">'. $category->content_en . '</div>';
            }
          }
          ?>
        </div>
       ...
       debugger;
           $.ajax({
               ...
               success: function(data) {
                   var counter=parseFloat($(".cart_counter").text());
